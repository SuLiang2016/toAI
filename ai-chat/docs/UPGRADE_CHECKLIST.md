...
- 测试框架选择：本轮遵守“不新增依赖”，采用 `node scripts/verify-upgrade.mjs` 作为可重复升级验证脚本；后续如需组件/E2E 覆盖再单独引入 Vitest/Playwright。
- Electron 生产模式：选择生产时在 Electron 主进程内启动嵌入式 Next 服务并加载本地 URL，保留动态 `/api/chat` 能力；不使用 `out/` 静态导出。
- 附件能力范围：默认不支持附件直传并给出明确错误；`AI_SUPPORTS_ATTACHMENTS=true` 或设置页启用后支持图片 `image_url` 直传，非图片文件提示先转文本摘要。
- 配置存储位置：API Key 继续只从 `.env.local` 读取；base URL、模型名和附件开关可通过 Electron preload/IPC 保存到本地用户数据目录，Web fallback 使用 `localStorage`。
- 自动更新：当前没有发布渠道和签名配置，只在 README 中记录策略；未硬接入 auto-update。

## 本轮验证记录

- 2026-05-18：升级前 `pnpm lint` 失败，既有问题包括 Electron CJS require lint、API/useChat `any`、ChatBox effect setState、`prefer-const`；升级后已修复。
- 2026-05-18：端口 `3000` 已由 Next 进程监听，且存在 Electron 进程；未停止用户现有进程。
- 2026-05-18：`.env.example` 仅包含占位值；`.env.local` 未写入文档或日志。
- 2026-05-18：Electron `path.txt` 存在于 `node_modules/.pnpm/electron@41.2.0/node_modules/electron/path.txt`。
- 2026-05-18：`pnpm lint` 通过。
- 2026-05-18：`pnpm typecheck` 通过。
- 2026-05-18：`pnpm test` 通过，输出 `upgrade verification checks passed`。
- 2026-05-18：`pnpm build` 通过，`/` 为静态页面，`/api/chat` 为动态路由。
- 2026-05-18：首次 `pnpm electron-build` 因从 GitHub 下载 Electron zip 出现 `EOF` 失败；已配置 `electronDist` 复用本地 Electron 分发目录。
- 2026-05-18：NSIS 安装器路径曾在 Windows `rcedit` 元数据写入阶段超时；`pnpm electron-build` 已调整为可重复的 `electron-builder --win dir`，安装器生成保留为 `pnpm electron-installer`。
- 2026-05-18：最终 `pnpm electron-build` 通过，产物位于 `dist/win-unpacked/AI Chat.exe`。
- 2026-05-18：Ralph deslop 后复验通过：`pnpm lint`、`pnpm typecheck`、`pnpm test`、`pnpm build`、`pnpm electron-build`、TypeScript project diagnostics 均为绿色。
